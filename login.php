...
if (isset($_POST['submit'])) {
    ...
    if (empty($username) || empty($password)) {
        echo '<script>alert("Isi username dan password dulu, ya.");</script>';
    } else {
        $password_hash = md5($password); 
        $query = mysqli_query($koneksi, "SELECT * FROM db_pengguna WHERE username='$username' AND password='$password_hash'");

        if (mysqli_num_rows($query) > 0) {
            $data = mysqli_fetch_array($query);
            $_SESSION['user'] = $data;             // ✅ seluruh data user
